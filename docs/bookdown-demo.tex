...
{\centering \includegraphics{bookdown-demo_files/figure-latex/ch3fig1-1} 
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.0465}}
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.0349}}
...
\hypertarget{applications-of-the-simple-regression-model}{%
\section{Applications of the simple regression model}\label{applications-of-the-simple-regression-model}}

\hypertarget{aggregate-consumption-function}{%
\subsection{Aggregate consumption function}\label{aggregate-consumption-function}}
...
\begin{rmdNote}
In macroeconomics, the above relationship is known as the \textbf{Keynesian} consumption function. John Maynard Keynes proposed that at the aggregate level, consumption changes in proportion to changes in disposable income:
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3824}}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3235}}
...
\hypertarget{returns-to-education}{%
\subsection{Returns to education}\label{returns-to-education}}
...
\begin{rmdNote}
The above formulation is a simplified version of the specification that has been extensively used in the labor economics literature. The above log-linear relationship between wages and education was made famous by Jacob Mincer and is commonly known as the \textbf{Mincerian earnings function}. The orginal specification also includes controls for years of experience. Using Census data from 1950 and 1960, he caluculated that every additional year of education increases annual earnings by 5 to 10\%.
\end{rmdNote}
...
\label{tab:ch3table6} OLS Estimation of Earnings Function
...
\hline \\[-1.8ex] 
 & \multicolumn{1}{c}{\textit{Dependent variable:}} \\ 
\cline{2-2} 
\\[-1.8ex] & Log of Annual Wages \\ 
\hline \\[-1.8ex] 
 Intercept & 9.259$^{***}$ \\ 
  & (0.135) \\ 
  & \\ 
 Years of Schooling & 0.106$^{***}$ \\ 
  & (0.009) \\ 
  & \\ 
\hline \\[-1.8ex] 
Observations & 1,000 \\ 
...
\hypertarget{use-of-natural-logs-and-interpretation-of-the-slope-coefficients}{%
\section{Use of natural logs and interpretation of the slope coefficients}\label{use-of-natural-logs-and-interpretation-of-the-slope-coefficients}}
...
\item
  \(ln(e^x)=e^{ln(x)}=x\)
\item
  If \(y=ln(x)\), then \(\displaystyle \frac{dy}{dx}=\frac{1}{x}\)
\item
  \(\Delta ln (X) \times 100 \approx \text{percent change in X}\). This implies that \(\displaystyle \frac{d ln(Y)}{d ln(X)}=\frac{\text{percent change in Y}}{\text{percent change in X}}\)
\end{enumerate}
\end{rmdNote}
...
\hypertarget{hypothesis-testing-and-confidence-interval-estimation}{%
\section{Hypothesis testing and confidence interval estimation}\label{hypothesis-testing-and-confidence-interval-estimation}}
...
\tightlist
\item
  Under the null hypothesis, and assuming that the error term is normally distributed, we can show that the above t-ratio follows a t-distribution with \(N-2\) degrees of freedom. The decision rule is given by:
\end{enumerate}
...
\begin{rmdNote}
\textbf{Test of Statistical Significance}: If we use \(R=0\), then the test becomes:
...
\def\labelenumi{\arabic{enumi}.}
\item
  Observed confounding factors: these, as the name suggests, are measurable relevant variables that can affect the outcome variable of interest. The idea here is that two observations may differ across obersvable dimensions which in turn influences the difference in their outcomes. This is known as \textbf{observed heterogeneity}. The multiple regression model can address this problem by adding all relevant and measurable independent variables to the right hand side of the regression model.
\item
  Unobserved confounding factors: these variables by definition are unobserved or difficult to measure in real world. The idea here is that two observations may differ across unobersvable dimensions which in turn influences the difference in their outcomes. This is known as \textbf{unobserved heterogeneity}. Going back to our wages regression model, it is possible that two inidividuals with same level of education and industry of employment earn different wages due to differences in their innate ability which is unobserved and very difficult to measure. There are some methods in Econometrics that address this problem and this problem posese one of the most serious challenge to the credibility of the estimated regression cofficients using the OLS. More on this later.
\end{enumerate}
\end{rmdNote}
...
\label{tab:ch5table1} OLS Estimation of Earnings Equation
...
\begin{rmdNote}
Note that for cross-sectional data correlation in error terms across observations is known as \textbf{spatial correlation}. In this course we will abstract away from this type of correlation and focus only on the serial correlation in time series data.
\end{rmdNote}
...
{\centering \includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-36-1} 
...
\label{tab:ch6table3} White's Standard Errors
...
Note that here \(t\) indexes the current period and \(s\) is an integer. For example, if \(s=1\), we are looking at correlation between \(y_{t}\) and \(y_{t-1}\). This is known as \textbf{first order} serial correlation. Similarly, for \(s=2\), we get the \textbf{second order} serial correlation between \(y_{t}\) and \(y_{t-2}\). In this ACF is a function of \(s\) and will give us a series of values of correlation of the current period with \(s\) past periods.
...
{\centering \includegraphics[width=0.3\linewidth]{bookdown-demo_files/figure-latex/tikz-ex-1} 
...
{\centering \includegraphics[width=0.3\linewidth]{bookdown-demo_files/figure-latex/tikz-ex2-1} 
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.1215}}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.4393}}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.4393}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
{\centering \includegraphics[width=0.3\linewidth]{bookdown-demo_files/figure-latex/tikz-ex3-1} 
...
{\centering \includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-84-1} 
...
{\centering \includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-85-1} 
...
\begin{center}\includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-86-1} \end{center}

\begin{center}\includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-87-1} \end{center}

\begin{center}\includegraphics{bookdown-demo_files/figure-latex/unnamed-chunk-88-1} \end{center}
...
\tightlist
\item
  The 95\% confidence interval for \(\widehat{\mu_X}\) is given by:
\end{enumerate}

\[\widehat{\mu_X}\pm \frac{\widehat{\sigma_X}}{\sqrt{N}} \times t_{c,2-sided}= 2\pm \frac{0.5}{\sqrt{20}}*2.093=\{1.766, 2.234\}\]

Hence, before we drew our sample from the population, there is a 95\% chance that the true population parameter (\(\mu_X\)) will fall between 1.766 and 2.234. Further, because zero is not inlcuded in the confidence band, we can say with 95\% confidence that the mean return on asset X is statistically significant.
...
\addcontentsline{toc}{section}{Table A: Critical Values for the t-distribution}
...
\addcontentsline{toc}{section}{Table B: Critical Values for the Chi-square distribution}
...
\addcontentsline{toc}{section}{Table C: 1\% Critical Values for the F distribution}
...
\addcontentsline{toc}{section}{Table D: 5\% Critical Values for the F distribution}
...
\addcontentsline{toc}{section}{Table E: 10\% Critical Values for the F distribution}
...
\addcontentsline{toc}{section}{Table F: 5\% One-sided Critical Values for the Durbin-Watson Distribution}
