...
### in the function below epsilon controls skewness and delta controls
### kurtosis..epsilon=0 and delta=1 gives normal distribution
...
2. Chi-square distribution: is obtained by squaring and adding independent standard normal distribution. For example, is $X$ and $Y$ are two standard normal random variables, then $Z=X^2 + Y^2$ follows a Chi-square distribution with two degrees of freedom.

3. F-distribution: is obtained by taking a ratio of two chi-square distribution. For example, if $X$ is Chi-square with $v_1$ degrees of freedom and $Y$ is a Chi-square with $v_2$ degrees of freedom, then $\displaystyle{Z=\frac{X}{Y}}$ follows F-distribution with $v_1$ and $v_2$ degrees of freedom.
...
2. Conditional distribution: For each random variable, we can also compute its probability distribution conditional on the other variable taking on a specific value. For example,e the conditional distribution of $Y$ given that $X=x$ is given by:
...
a. Show that $\widehat{\mu_A}=0.25\times X_1 +0.25\times X_3+ 0.25 \times X_8 + 0.25 X_{20}$ is an unbiased estimator of $\mu$.

b. Show that $\widehat{\mu_B}=0.1\times X_1 +0.1\times X_3+ 0.5 \times X_8+0.3 \times X_{11}$ is an unbiased estimator of $\mu$.

c. Now compute variance of $\widehat{\mu_A}$ and $\widehat{\mu_B}$. Which one is more efficient estimator of $\mu$.
...
c. Compute the 95\% confidence interval for $\widehat{\mu_X}$. Using the confidence interval, what can you say about the population mean return for fund $X$?
...
a. \[E(\widehat{\mu_A})=0.25\times E(X_1) +0.25\times E(X_3)+ 0.25 \times E(X_8) + 0.25 E(X_{20})\]
...
b. \[E(\widehat{\mu_B})=0.1\times E(X_1)+0.1\times E(X_3)+ 0.5 \times E(X_8)+0.3 \times E(X_{11})\]
